...
using Management.Comport.Views.Dialogs;
...
namespace Management.Comport.ViewModels
{
    internal class ComportSettingViewModel : BaseRegionViewModel
    {
        ...
        private async void OnSend()
        {
            ...
            {
                case Core.Constants.DeviceType.TextCommand:
                    var textSession = await CurrentDevice.SendAndWait(TextSession);
                    if (textSession == null)
                    {
                        return;
                    }
                    if (textSession.Result == Core.Constants.ResultType.NG)
                    {
                ...
                case Core.Constants.DeviceType.ByteCommand:
                    var session = await CurrentDevice.SendAndWait(TextSession);
                    if (session.Result == Core.Constants.ResultType.NG)
                    {
